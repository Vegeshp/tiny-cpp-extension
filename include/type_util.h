...
struct RealType {
    static std::string name() {
        std::string type_str =
#ifndef FILT
                        abi::__cxa_demangle(typeid(std::remove_cv_t<T>).name(),
...
std::string iterate_params() {
    return (std::is_same<T, void>{}() ? "" : __detail::get_name<T>())
        .append(")");
}

template <typename T, typename... Args>
std::enable_if_t<sizeof...(Args) >= 1, std::string> iterate_params() {
    return __detail::three_parts<T>(", ", iterate_params<Args...>());
}

template <typename T>
std::string to_array(const std::string& ops, size_t size) {
    return __detail::three_parts<T>(" (" + ops + ") [", std::to_string(size))
        .append("]");
}
